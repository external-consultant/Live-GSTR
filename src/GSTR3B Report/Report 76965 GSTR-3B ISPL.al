report 76965 "GSTR-3B ISPL"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Source Code/GSTR3B Report/GSTR3B.rdl';
    ...
    var
        ...
        KFLOODCESSCompLbl: Label 'KFLOODCESS', Locked = true;
    ...
    begin
        ...
        //NJ-NS
        if DetailedGSTLedgerEntry."GST Component Code" = KFLOODCESSCompLbl then
            exit;
        //NJ-NE
    ...
    var
        DetailedGSTLedgerEntry: Record "Detailed GST Ledger Entry";
        DetailedGSTLedgerEntryInfo: Record "Detailed GST Ledger Entry Info";
        DtldCrAdjstmntEntry_lRec: Record "Detailed Cr. Adjstmnt. Entry";
    begin
        // Eligible ITC
        // Import of Goods
        ...
        // DetailedGSTLedgerEntry.SetRange("Source Type", "Source Type"::Vendor);  //NJ-O
        DetailedGSTLedgerEntry.SetRange("Input Service Distribution", false); //NJ-N
            ...
            repeat
                ...
                if DetailedGSTLedgerEntryInfo.FindSet() then
                    if (DetailedGSTLedgerEntry."GST Vendor Type" = DetailedGSTLedgerEntry."GST Vendor Type"::SEZ)
                            and (DetailedGSTLedgerEntry."GST Without Payment of Duty" = false) then begin            //NJ-N              
                        //   and (DetailedGSTLedgerEntryInfo."Without Bill Of Entry" = false) then begin  //NJ-O
                        ImportGoodsIGSTAmount += GetSupplyGSTAmountLine(DetailedGSTLedgerEntry, IGSTLbl);
                        ImportGoodsCGSTAmount += GetSupplyGSTAmountLine(DetailedGSTLedgerEntry, CGSTLbl);
                        ImportGoodsSGSTUTGSTAmount += GetSupplyGSTAmountLine(DetailedGSTLedgerEntry, SGSTLbl);
                        ImportGoodsCESSAmount += GetSupplyGSTAmountLine(DetailedGSTLedgerEntry, CESSCompLbl);
        ...
        //Update From Detailed Cr Adjustment Entry
        DtldCrAdjstmntEntry_lRec.Reset();
        DtldCrAdjstmntEntry_lRec.SetRange("Location  Reg. No.", GSTIN);
        DtldCrAdjstmntEntry_lRec.SetRange("Posting Date", StartingDate, EndingDate);
        DtldCrAdjstmntEntry_lRec.SetFilter("Credit Adjustment Type", '%1|%2', DtldCrAdjstmntEntry_lRec."Credit Adjustment Type"::"Credit Availment", DtldCrAdjstmntEntry_lRec."Credit Adjustment Type"::"Credit Re-Availment");
        DtldCrAdjstmntEntry_lRec.SetRange("Adjusted Doc. Transaction Type", DtldCrAdjstmntEntry_lRec."Adjusted Doc. Transaction Type"::Purchase);
        DtldCrAdjstmntEntry_lRec.SetFilter("GST Vendor Type", '%1|%2', "GST Vendor Type"::Import, DtldCrAdjstmntEntry_lRec."GST Vendor Type"::SEZ);
        DtldCrAdjstmntEntry_lRec.SetRange("GST Group Type", DtldCrAdjstmntEntry_lRec."GST Group Type"::Goods);
        if DtldCrAdjstmntEntry_lRec.FindSet() then
            repeat
                ImportGoodsIGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, IGSTLbl);
                ImportGoodsCGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, CGSTLbl);
                ImportGoodsSGSTUTGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, SGSTLbl);
                ImportGoodsCESSAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, CESSCompLbl);
            until DtldCrAdjstmntEntry_lRec.Next() = 0;
        ...
        DtldCrAdjstmntEntry_lRec.SetRange("Adjusted Doc. Transaction Type", DtldCrAdjstmntEntry_lRec."Adjusted Doc. Transaction Type"::Purchase);
        DtldCrAdjstmntEntry_lRec.SetFilter("GST Vendor Type", '%1|%2', "GST Vendor Type"::Import, DtldCrAdjstmntEntry_lRec."GST Vendor Type"::SEZ);
        DtldCrAdjstmntEntry_lRec.SetRange("GST Group Type", DtldCrAdjstmntEntry_lRec."GST Group Type"::Service);
        if DtldCrAdjstmntEntry_lRec.FindSet() then
            repeat
                Sign := -1;
                ImportServiceIGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, IGSTLbl) * Sign;
                ImportServiceCGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, CGSTLbl) * Sign;
                ImportServiceSGSTUTGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, SGSTLbl) * Sign;
                ImportServiceCESSAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, CESSCompLbl) * Sign;
            until DtldCrAdjstmntEntry_lRec.Next = 0;
        ...
        if DetailedGSTLedgerEntry.FindSet() then
            repeat
                if Not (DetailedGSTLedgerEntry."GST Vendor Type" in [DetailedGSTLedgerEntry."GST Vendor Type"::Import, DetailedGSTLedgerEntry."GST Vendor Type"::SEZ]) then begin
        ...
        DtldCrAdjstmntEntry_lRec.Reset();
        DtldCrAdjstmntEntry_lRec.SetCurrentKey("Location  Reg. No.", "Posting Date", "Reverse Charge");
        DtldCrAdjstmntEntry_lRec.SetRange("Location  Reg. No.", GSTIN);
        DtldCrAdjstmntEntry_lRec.SetRange("Posting Date", StartingDate, EndingDate);
        DtldCrAdjstmntEntry_lRec.SetFilter("Credit Adjustment Type", '%1|%2', DtldCrAdjstmntEntry_lRec."Credit Adjustment Type"::"Credit Availment", DtldCrAdjstmntEntry_lRec."Credit Adjustment Type"::"Credit Re-Availment");
        DtldCrAdjstmntEntry_lRec.SetRange("Adjusted Doc. Transaction Type", DtldCrAdjstmntEntry_lRec."Adjusted Doc. Transaction Type"::Purchase);
        // DtldCrAdjstmntEntry_lRec.SetFilter("GST Vendor Type", '<>%1', "GST Vendor Type"::Import);
        DtldCrAdjstmntEntry_lRec.SetRange("Reverse Charge", true);
        if DtldCrAdjstmntEntry_lRec.Findset() then
            repeat
                if Not (DtldCrAdjstmntEntry_lRec."GST Vendor Type" in [DtldCrAdjstmntEntry_lRec."GST Vendor Type"::Import, DtldCrAdjstmntEntry_lRec."GST Vendor Type"::SEZ]) then begin
                    InwrdReverseIGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, IGSTLbl) * Sign;
                    InwrdReverseCGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, CGSTLbl) * Sign;
                    InwrdReverseSGSTUTGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, SGSTLbl) * Sign;
                    InwrdReverseCESSAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, CESSCompLbl) * Sign;
                end;
            until DtldCrAdjstmntEntry_lRec.Next = 0;
    end;
    ...
    begin
        ...
        */
        ...
        DtldCrAdjstmntEntry_lRec.SetFilter("Credit Adjustment Type", '%1|%2', DtldCrAdjstmntEntry_lRec."Credit Adjustment Type"::"Credit Availment", DtldCrAdjstmntEntry_lRec."Credit Adjustment Type"::"Credit Re-Availment");
        DtldCrAdjstmntEntry_lRec.SetRange("Adjusted Doc. Transaction Type", DtldCrAdjstmntEntry_lRec."Adjusted Doc. Transaction Type"::Purchase);
        DtldCrAdjstmntEntry_lRec.SetRange("GST Vendor Type", "GST Vendor Type"::Registered);
        DtldCrAdjstmntEntry_lRec.SetRange("Reverse Charge", false);
        if DtldCrAdjstmntEntry_lRec.FindSet() then
            repeat
                AllOtherITCIGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, IGSTLbl);
                AllOtherITCCGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, CGSTLbl);
                AllOtherITCSGSTUTGSTAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, SGSTLbl);
                AllOtherITCCESSAmount += GetSupplyGSTAmountDCrAdjmntLine(DtldCrAdjstmntEntry_lRec, CESSCompLbl);
            until DtldCrAdjstmntEntry_lRec.Next() = 0;

        DetailedCrAdjstmntEntry.Reset();
        DetailedCrAdjstmntEntry.SetCurrentKey("Location  Reg. No.", "Posting Date");
        DetailedCrAdjstmntEntry.SetRange("Location  Reg. No.", GSTIN);
        DetailedCrAdjstmntEntry.SetRange("Posting Date", StartingDate, EndingDate);
        ...
        //NJ-OE
        //NJ-NS
        DetailedCrAdjstmntEntry.SetFilter("Credit Adjustment Type", '%1|%2|%3', DetailedCrAdjstmntEntry."Credit Adjustment Type"::"Credit Reversal", DetailedCrAdjstmntEntry."Credit Adjustment Type"::"Permanent Reversal", DetailedCrAdjstmntEntry."Credit Adjustment Type"::"Reversal of Availment");
        //NJ-NE
        if DetailedCrAdjstmntEntry.FindSet() then
            repeat
                OthersIGSTAmount += ABS(GetSupplyGSTAmountDCrAdjmntLine(DetailedCrAdjstmntEntry, IGSTLbl));
                OthersCGSTAmount += ABS(GetSupplyGSTAmountDCrAdjmntLine(DetailedCrAdjstmntEntry, CGSTLbl));
                OthersSGSTUTGSTAmount += ABS(GetSupplyGSTAmountDCrAdjmntLine(DetailedCrAdjstmntEntry, SGSTLbl));
                OthersCESSAmount += ABS(GetSupplyGSTAmountDCrAdjmntLine(DetailedCrAdjstmntEntry, CESSCompLbl));
            until DetailedCrAdjstmntEntry.Next() = 0;
        ...
        if DetailedGSTLedgerEntry.FindSet() then
            repeat
                OthersIGSTAmount += ABS(GetSupplyGSTAmountLine(DetailedGSTLedgerEntry, IGSTLbl));
                OthersCGSTAmount += ABS(GetSupplyGSTAmountLine(DetailedGSTLedgerEntry, CGSTLbl));
                OthersSGSTUTGSTAmount += ABS(GetSupplyGSTAmountLine(DetailedGSTLedgerEntry, SGSTLbl));
                OthersCESSAmount += ABS(GetSupplyGSTAmountLine(DetailedGSTLedgerEntry, CESSCompLbl));
            until DetailedGSTLedgerEntry.Next() = 0;
    end;
    ...
    begin
        ...
        GSTAmountGSTR3B.SetFilter(GSTAmountGSTR3B.GST_Component_Code, '<>%1&<>%2', CESSCompLbl, KFLOODCESSCompLbl); //NJ
        GSTAmountGSTR3B.SetFilter(Component_Calc__Type, '%1|%2|%3',
          "Component Calc Type"::General,
          "Component Calc Type"::Threshold,
          "Component Calc Type"::"Cess %");
    ...
    internal procedure SetJustFillData_gFnc(GSTCode_iCde: Code[20]; PeriodDate_iDte: Date; AuthorisedPerson_iTxt: Text[100]; Place_iTxt: Text[50]; PostingDate_iDte: Date)
    begin
        GSTIN := GSTCode_iCde;
        PeriodDate := PeriodDate_iDte;
        AuthorisedPerson := AuthorisedPerson_iTxt;
        Place := Place_iTxt;
        PostingDate := PostingDate_iDte;
    end;
}