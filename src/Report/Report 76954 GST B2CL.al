Report 76954 "GST B2CL"
{
    ...
    dataset
    {
        dataitem("Detailed GST Ledger Entry"; "Detailed GST Ledger Entry")
        {
            ...
            begin
                ...
                SetFilter("GST Component Code", '=%1', 'IGST');  //Only Inter State Transcation
                ...
                GSTB2CL_gRec.Reset;
                GSTB2CL_gRec.SetCurrentkey("GST Month", "GST Year");
                GSTB2CL_gRec.SetRange("GST Month", Month_gInt);
                GSTB2CL_gRec.SetRange("GST Year", Year_gInt);
                GSTB2CL_gRec.SetRange("GST Registration No.", GSTRegNo_gCod);
                GSTB2CL_gRec.DeleteAll;
            end;
        }
    }
    ...
    var
        GSTB2CL_lXml: XmlPort "GST - B2CL Export";
        GSTB2CL_lRec: Record "GST B2CL";
    ...
    begin
        ...
        if not JustFillData_gBln then begin
            GSTB2CL_lRec.Reset;
            GSTB2CL_lRec.SetRange("GST Month", Month_gInt);
            GSTB2CL_lRec.SetRange("GST Year", Year_gInt);
            GSTB2CL_lRec.SetRange("GST Registration No.", GSTRegNo_gCod);
            if ExportToExcel_gBln then begin
                MakeExcelDataInfo_lFnc;
                if GSTB2CL_lRec.FindSet then begin
                    DataExported_gBln := true;
                    if ExcelSaveFilePath_gTxt <> '' then
                        MakeExcelDataHeader_lFnc;
                    repeat
                        MakeExcelDataBody_lFnc(GSTB2CL_lRec);
                    until GSTB2CL_lRec.Next = 0;
                end;
                CreateExcelBook_lFnc;
            end else begin
                Clear(GSTB2CL_lXml);
                GSTB2CL_lXml.SetTableview(GSTB2CL_lRec);

                if SetOutPutFile_gTxt = '' then begin
                    GSTB2CL_lXml.Run;
                end else begin
                    if GSTB2CL_lRec.IsEmpty then
                        CurrReport.Break;
                    ...
                    GSTB2CL_lXml.HideWinDialog_gFnc;
                    GSTB2CL_lXml.SetDestination(OutStream_lOST);
                    GSTB2CL_lXml.Export;
                    DataExported_gBln := true;
                end;
            end;
    ...
    begin
        ...
        GSTB2CL_lRec.Reset;
        GSTB2CL_lRec.SetCurrentkey("Invoice Number", Rate);
        GSTB2CL_lRec.SetRange("Invoice Number", DetailedGSTLedgerEntry_iRec."Document No.");
        GSTB2CL_lRec.SetRange(Rate, DetailedGSTLedgerEntry_iRec."GST %");
        if not GSTB2CL_lRec.FindFirst then begin
            GSTB2CL_lRec.Init;
            GSTB2CL_lRec."GST Month" := Month_gInt;
            GSTB2CL_lRec."GST Year" := Year_gInt;
            GSTB2CL_lRec."GST Registration No." := DetailedGSTLedgerEntry_iRec."Location  Reg. No.";
            GSTB2CL_lRec."Invoice Number" := DetailedGSTLedgerEntry_iRec."Document No.";
            GSTB2CL_lRec."Invocie Date" := DetailedGSTLedgerEntry_iRec."Posting Date";
            GSTB2CL_lRec."Invoice Value" := GSTRCommonFunction_gCdu.FindInvoiceValue_gFnc(DetailedGSTLedgerEntry_iRec);
            GSTB2CL_lRec."Place of Supply" := GSTRCommonFunction_gCdu.GSTRPlaceOfSupply_gFnc(DetailedGSTLedgerEntry_iRec);
            // GSTB2CL_lRec."E-Commerce GSTIN" := DetailedGSTLedgerEntry_iRec."e-Comm. Operator GST Reg. No.";
            GSTB2CL_lRec.Rate := DetailedGSTLedgerEntry_iRec."GST %";
            GSTB2CL_lRec.Insert;
        end;

        GSTB2CL_lRec."Taxable Value" := GSTB2CL_lRec."Taxable Value" + Abs(DetailedGSTLedgerEntry_iRec."GST Base Amount");
        GSTB2CL_lRec.Modify;
    end;
    ...
    begin
        ...
        ExcelBuffer_gRecTmp.AddInfoColumn('GST B2CL', false, false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Text);
        ExcelBuffer_gRecTmp.NewRow;
        ExcelBuffer_gRecTmp.AddInfoColumn(Format(Text006), false, true, false, false, '', ExcelBuffer_gRecTmp."cell type"::Text);
        ExcelBuffer_gRecTmp.AddInfoColumn(Report::"GST B2CL", false, false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Number);
        ExcelBuffer_gRecTmp.NewRow;
    ...
    local procedure MakeExcelDataBody_lFnc(GSTB2CL_iRec: Record "GST B2CL")
    var
    ...
    begin
        ExcelBuffer_gRecTmp.NewRow;
        ExcelBuffer_gRecTmp.AddColumn(GSTB2CL_iRec."Invoice Number", false, '', false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Text);
        ExcelBuffer_gRecTmp.AddColumn(GSTB2CL_iRec."Invocie Date", false, '', false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Date);
        ExcelBuffer_gRecTmp.AddColumn(GSTB2CL_iRec."Invoice Value", false, '', false, false, false, '#,##0.00', ExcelBuffer_gRecTmp."cell type"::Number);
        ExcelBuffer_gRecTmp.AddColumn(GSTB2CL_iRec."Place of Supply", false, '', false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Text);
        ExcelBuffer_gRecTmp.AddColumn('', false, '', false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Text);
        ExcelBuffer_gRecTmp.AddColumn(GSTB2CL_iRec.Rate, false, '', false, false, false, '#,##0.00', ExcelBuffer_gRecTmp."cell type"::Number);
        ExcelBuffer_gRecTmp.AddColumn(GSTB2CL_iRec."Taxable Value", false, '', false, false, false, '#,##0.00', ExcelBuffer_gRecTmp."cell type"::Number);
        ExcelBuffer_gRecTmp.AddColumn(GSTB2CL_iRec."Cess Amount", false, '', false, false, false, '#,##0.00', ExcelBuffer_gRecTmp."cell type"::Number);
        ExcelBuffer_gRecTmp.AddColumn(GSTB2CL_iRec."E-Commerce GSTIN", false, '', false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Text);
    end;

    local procedure CreateExcelBook_lFnc()
    begin
        ExcelBuffer_gRecTmp.CreateNewBook('GST B2CL');
        ExcelBuffer_gRecTmp.WriteSheet('Data', CompanyName(), UserId());
        ExcelBuffer_gRecTmp.CloseBook();
        ExcelBuffer_gRecTmp.OpenExcel();

        // if ExcelSaveFilePath_gTxt = '' then
        //     ExcelBuffer_gRecTmp.CreateBookAndOpenExcel('', 'GST B2CL', 'Data', COMPANYNAME, UserId)
        // else begin
        //     ExcelBuffer_gRecTmp.FixLocationCreateBookAndSaveExcelMailSharedFile_gFnc('GST B2CL', 'Data', ExcelSaveFilePath_gTxt);
    end;
